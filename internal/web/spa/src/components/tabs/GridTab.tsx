  ...
  return (
    <div className="grid-pane">
      <div className="grid-toolbar">
        <button className="tb-ico" title="refresh" onClick={() => load(page, where, order)}>↻</button>
        <span className="tb-sep" />
        <button className="tb-ico" title="add row (coming soon)" disabled>＋</button>
        <button className="tb-ico" title="remove row (coming soon)" disabled>－</button>
        <span className="tb-sep" />
                    ...
                    {(result.columns || []).map((c, i) => (
                      <th key={i}>
                        ...
                        <span className="th-tools">
                          <a className="th-sort" title="sort asc" onClick={() => sort(c, 'asc')}>▲</a>
                          <a className="th-sort" title="sort desc" onClick={() => sort(c, 'desc')}>▼</a>
                        </span>
                      </th>
                    ))}
                  </tr></thead>
      ...
      <div className="grid-footer hud-label">
        {conn && <span className={`ico ico-${conn.kind}`} />}
        <span>{schema}.{table}</span>
        <span className="tb-grow" />
        {page > 0 && <a className="pg-btn" onClick={() => setPage(p => p - 1)}>‹ prev</a>}
        <span>page {page + 1}</span>
        {rows.length > 0 && <a className="pg-btn" onClick={() => setPage(p => p + 1)}>next ›</a>}
      </div>
    </div>
  )
}
